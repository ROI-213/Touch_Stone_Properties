...
export function ListingPage({ listingType, type, initialQuery = "" }: Props) {
  ...
  const compareProps = compare
    .map((id) => liveProperties.find((p) => p.id === id))
  ...
  return (
    <div className="min-h-screen bg-ivory">
      ...
      <AnimatePresence>
        {compare.length >= 2 && (
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-40 border-t border-gold-light/40 bg-gold px-4 py-3 text-charcoal shadow-elevated"
          >
            <div className="mx-auto flex w-full flex-wrap items-center gap-3 px-4 md:px-8">
              <span className="text-sm font-semibold">
                Comparing {compare.length} properties:
              </span>
              <div className="flex flex-1 flex-wrap gap-2">
                {compare.map((id) => {
                  const p = liveProperties.find((x) => x.id === id);
                  if (!p) return null;
                  return (
                    <span
                      key={id}
                      className="inline-flex items-center gap-1 rounded-full bg-charcoal/10 px-3 py-1 text-xs font-medium"
                    >
                      {p.title}
                      <button
              ...
              <button
                onClick={() => setCompare([])}
                className="rounded-full border border-charcoal/30 px-3 py-1.5 text-xs font-medium hover:bg-charcoal/10"
              >
                Clear
              </button>
              <button
                onClick={openCompare}
                className="rounded-full bg-charcoal px-4 py-2 text-xs font-semibold text-gold hover:bg-stone-brand"
              >
                Compare Now →
              </button>
            </div>
      ...
      <CompareModal
        ...
        onRemove={(id) => {
          toggleCompare(id);
          if (compareProps.length <= 2) setCompareOpen(false);
        }}
      />
