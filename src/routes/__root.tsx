...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  useEffect(() => {
    ...
    const attempts = Number(window.sessionStorage.getItem(retryKey) ?? "0");
    const MAX_ATTEMPTS = 4;
    if (attempts >= MAX_ATTEMPTS) return;

    // Exponential backoff: 400ms, 800ms, 1600ms, 3200ms (+ jitter)
    const delay = Math.min(400 * 2 ** attempts, 3200) + Math.floor(Math.random() * 200);
