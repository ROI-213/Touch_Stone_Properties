  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      if (data.employee_code) {
        const { count } = await supabaseAdmin.from("staff_users").select("*", { count: "exact", head: true }).eq("employee_code", data.employee_code);
        if (count && count > 0) throw new Error("Employee code already in use");
      }
      if (data.username) {
        const { count } = await supabaseAdmin.from("staff_users").select("*", { count: "exact", head: true }).eq("username", data.username);
      ...
      const { data: staffRow, error: insertErr } = await supabaseAdmin
        ...
        .select("id")
        .single();
      if (insertErr || !staffRow) throw new Error(insertErr?.message ?? "Failed to insert staff user");

      // Assign 'staff' role (also keep default 'customer' from trigger — that's harmless)
...
export const deleteStaffUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // 1. Load the staff record using staffId
    ...
    // Prevent deletion of the final super admin account
    ...
    // 2. Try to delete the Supabase Auth user
    let authUserAlreadyMissing = false;
    const { error: authError } = await supabaseAdmin.auth.admin.deleteUser(authUserId);
    
    if (authError) {
      // If the error indicates the user doesn't exist, we treat it as an orphaned record
      if (authError.message.toLowerCase().includes("user not found")) {
        authUserAlreadyMissing = true;
      } else {
        // Any other Auth error stops the deletion safely
        throw new Error(`Auth deletion failed: ${authError.message}`);
      }
    }

    // 3. Delete the staff_users row explicitly (in case CASCADE didn't trigger because Auth user was missing)
